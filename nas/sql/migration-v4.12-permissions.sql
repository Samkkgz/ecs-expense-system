...
-- 执行：PGPASSWORD=... psql -h 127.0.0.1 -p 15432 -U postgres -d postgres -f migration-v4.12-permissions.sql
-- 安全说明：脚本幂等，可重复执行

BEGIN;

-- 1) 新用户自动创建 member 档案（不再允许前端自动建 super_admin）
...
-- 2) 费用类目：成员只读，管理员/服务角色可写
DROP POLICY IF EXISTS "categories_all" ON public.expense_categories;
DROP POLICY IF EXISTS "categories_read" ON public.expense_categories;
DROP POLICY IF EXISTS "categories_write" ON public.expense_categories;
...
-- 3) 发票：成员只能看/存/提交自己公司的发票，不能审批、不能删除
...
-- 5) 档案：成员只能看自己的档案，管理员可看全部；插入仅限管理员
...
-- 7) Storage：成员只能访问/上传自己公司的对象，删除/更新仅管理员
DROP POLICY IF EXISTS "authenticated_all_objects" ON storage.objects;
...
-- 8) 权限判断辅助函数（SECURITY DEFINER 内部使用）
...
CREATE OR REPLACE FUNCTION public.admin_assign_user_companies(p_user_id UUID, p_company_ids BIGINT[])
RETURNS void LANGUAGE plpgsql SECURITY DEFINER AS $fn$
BEGIN
  IF NOT public.is_admin_or_service() THEN
    RAISE EXCEPTION '无用户管理权限';
...
-- 10) 创建/邀请用户（返回一次性初始密码；已存在则仅更新角色与公司）
...
-- 11) 兼容历史遗留：无档案用户登录时只允许自建 member 档案
